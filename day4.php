<?php
  ...
  */
  ...
  foreach($temp_arr2 as $ta2){
    if(substr_count($ta2, ':') <= 6) continue;

    $break = 0;
    foreach($rules as $must_have){
      if(strpos($ta2, ($must_have.':')) === false){
        $break = 1;
        break;
      }
    }
  ...
  */
  ...
  /* for my sane mind */
  $temp_arr4 = [];
  foreach($temp_arr3 as $ta3){
    $temp_arr5 = explode(' ', $ta3);
    $temp_arr6 = [];
    foreach($temp_arr5 as $ta5) {
      $temp_arr7 = explode(':', $ta5);
      if($temp_arr7[0] != 'cid')
        $temp_arr6[$temp_arr7[0]] = $temp_arr7[1];
    }
    ksort($temp_arr6);
    $temp_arr4[] = $temp_arr6;
  }
  
  foreach($temp_arr4 as $ta4){
    $break2 = 0;

    foreach($ta4 as $key => $val){
      if($key == 'byr'
        && (!ctype_digit($val) || $val < 1920 || $val > 2002)){
        $break2 = 1;
        break;
      }

      if($key == 'iyr'
        && (!ctype_digit($val) || $val < 2010 || $val > 2020)){
        $break2 = 1;
        break;
      }

      if($key == 'eyr'
        && (!ctype_digit($val) || $val < 2020 || $val > 2030)){
        $break2 = 1;
        break;
      }
      ...
      if($key == 'hgt'){
        if(strpos($val, 'cm') !== false){
          $temp_str4 = str_replace('cm','',$val);
          if($temp_str4 < 150 || $temp_str4 > 193){
            $break2 = 1;
            break;
          }
        }
        elseif(strpos($val, 'in') !== false){
          $temp_str4 = str_replace('in','',$val);
          if($temp_str4 < 59 || $temp_str4 > 76){
            $break2 = 1;
            break;
          }
        }
