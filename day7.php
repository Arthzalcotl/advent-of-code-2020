<?php
  $raw='plaid fuchsia bags contain 5 light violet bags, 1 light yellow bag.
  ...
  */
  $temp_arr1 = explode("\n", $raw);
  $temp_arr2 = [];
  foreach($temp_arr1 as $ta1){
    $temp_str1 = str_replace(['bags','bag','.'], '', $ta1);
    $temp_arr3 = explode('contain', $temp_str1);
    $temp_arr4 = explode(',', $temp_arr3[1]);

    $temp_arr3[0] = trim($temp_arr3[0]);
    $temp_arr3[1] = array_map('trim', $temp_arr4);
  ...
  */
  $checked_bags = [];
  $bags = [];
  foreach($temp_arr2 as $ta2){
    foreach($ta2[1] as $ta2_contain){
      if(strpos($ta2_contain, $your_bag) !== false){
        $bags[] = $ta2[0];
        break;
      }
    }
  ...
  function loopOutLoop($bag){
    global $temp_arr2;
    global $checked_bags;
    
    foreach($temp_arr2 as $value){
      foreach($value[1] as $value_contain){
        if(strpos($value_contain, $bag) !== false){
          loopOutLoop(trim($value[0]));
          $checked_bags[] = trim($value[0]);
          break;
        }
      }
    }
  ...
  echo count(array_unique($checked_bags));

  /*
    2nd half
  */
  /*
    no need to code something just to get this particular line ah
    simple text search would suffice
  */
  $your_bag_str = 'shiny gold bags contain 4 drab blue bags, 4 posh purple bags, 2 drab silver bags, 4 wavy turquoise bags.';
  $your_bag_contain_arr = [
    'drab blue' => 4,
    'posh purple' => 4,
    'drab silver' => 2,
    'wavy turquoise' => 4
  ...
  function loopInLoop($bag, $num){
    ...
    foreach($temp_arr2 as $value){
      if($value[0] == $bag){
        foreach($value[1] as $value_contain){
          if($value_contain == 'no other') break;

          $number_value = substr($value_contain, 0, strpos($value_contain, ' '));
          $bag_value = trim(substr($value_contain, strpos($value_contain, ' ')));
          
          $total_bags += $number_value * $num;
          loopInLoop(trim($bag_value), $number_value * $num);
        }
        break;
      }
    }
  ...
  foreach($your_bag_contain_arr as $key=>$val){
    $total_bags += $val;
    loopInLoop(trim($key), $val);
  }
  echo $total_bags;